...
export default function AboutUs() {
  ...
  return (
    <div id="about-view-scene" className="pt-[110px] pb-24 max-w-7xl mx-auto px-4 md:px-8 w-full flex flex-col gap-16 md:gap-24">
      
      {/* 1. HERO STORY BANNER */}
      <section className="bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-16 lg:p-20 relative overflow-hidden text-center md:text-left flex flex-col md:flex-row items-center gap-10">
        ...
        <div className="flex-1 flex flex-col gap-5">
          ...
          <h1 className="text-4xl md:text-5xl font-black font-display text-white tracking-tight leading-none uppercase">
            We live and<br />
        ...
        <div className="flex-1 w-full flex items-center justify-center">
          <img 
            ...
            className="rounded-2xl border border-slate-850 shadow-2xl w-full max-w-md object-cover transform rotate-1 hover:rotate-0 duration-300 pointer-events-none"
      ...
      <section id="about-stats-strip" className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {stats.map((st, id) => (
          <div key={id} className="bg-slate-900/40 border border-slate-805 p-6 rounded-2xl text-center flex flex-col gap-1.5 relative group">
            <span className="text-3xl md:text-4xl font-black font-display text-blue-400 glow-text-blue">{st.title}</span>
            <hr className="w-10 border-slate-800 mx-auto" />
      ...
      <section className="flex flex-col gap-12">
        <div className="text-center flex flex-col gap-2">
          ...
          <p className="text-2xl md:text-3xl font-bold font-display text-white">Why GadgetsKini Leads</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {valuesCard.map((card, i) => (
            <div key={i} className="bg-slate-900 border border-slate-800 p-6 md:p-8 rounded-2xl text-left flex flex-col gap-4 relative">
              <div className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl w-fit">
                {card.icon}
              </div>
              <h3 className="text-sm font-bold font-display text-white uppercase tracking-wider">{card.title}</h3>
      ...
      <section className="flex flex-col gap-12 pb-10">
        <div className="text-center flex flex-col gap-2">
          ...
          <h2 className="text-2xl md:text-3xl font-bold font-display text-white">Meet Our Curators</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {TEAM_MEMBERS.map((member) => (
            <div 
              key={member.id}
              className="bg-slate-900/60 border border-slate-850 p-6 rounded-2xl flex flex-col items-center text-center gap-4 group hover:border-slate-750 duration-200"
            >
              <div className="h-28 w-28 rounded-full border border-slate-800 overflow-hidden relative">
                <img 
                  src={member.avatar} 
                  alt={member.name} 
              ...
              <div>
                <h4 className="font-display font-black text-sm text-white">{member.name}</h4>
