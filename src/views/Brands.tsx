...
export default function Brands() {
  ...
  const brandList: BrandMeta[] = [
    ...
    },
    {
      ...
      icon: <Cpu className="text-slate-600 group-hover:scale-110 transition-transform duration-300" size={24} />
    },
    {
      name: 'Samsung',
  ...
  return (
    <div id="brands-view-container" className="pb-24 max-w-7xl mx-auto px-4 md:px-8 w-full flex flex-col gap-12 text-left">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-slate-200 pb-10">
        <div className="flex flex-col gap-3 max-w-2xl">
          ...
          <h1 className="text-4xl md:text-6xl font-black font-display text-slate-900 uppercase tracking-tight leading-none">
            Our Partner Brands
          </h1>
        ...
        <div className="relative w-full lg:w-80 border-slate-200">
          <input
            id="brand-search"
            type="text"
            ...
            className="w-full bg-white/90 text-xs border border-slate-750 text-slate-900 rounded-xl pl-10 pr-4 py-3 outline-none focus:border-cyan-500 focus:bg-slate-50 transition-colors"
          />
      ...
      <div id="brands-grid" className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-mono font-bold uppercase tracking-widest text-slate-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <button
                  ...
                  className={`group text-left p-5 rounded-2xl border transition-all duration-300 relative overflow-hidden flex flex-col justify-between min-h-[170px] cursor-pointer ${
                    isSelected 
                      ...
                      : 'bg-white/50 hover:bg-slate-50 border-slate-200'
                  }`}
                >
                  ...
                  <div className="flex items-start justify-between gap-4 w-full">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-lg border transition-colors ${
                        isSelected ? 'bg-cyan-500/10 border-cyan-500/30' : 'bg-white border-slate-200'
                      }`}>
                        {brand.icon}
                      </div>
                      <div>
                        <h3 className="text-lg font-bold font-display tracking-tight text-slate-900 uppercase">
                          {brand.name}
                        </h3>
                  ...
                  {/* Subtitle Tagline */}
                  <p className="text-xs text-slate-500 leading-snug font-sans mt-3 line-clamp-2">
                    {brand.tagline}
                  </p>

                  {/* Micro stats strip */}
                  <div className="flex items-center justify-between border-t border-slate-200/80 pt-3 mt-4 w-full text-[10px] font-mono text-slate-500">
            ...
            {filteredBrands.length === 0 && (
              <div className="col-span-2 p-12 text-center border border-dashed border-slate-200 rounded-2xl text-slate-500 text-xs py-16">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="bg-slate-50 border border-slate-200/90 rounded-2xl p-6 md:p-8 flex flex-col gap-6"
            >
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`px-2 py-0.5 rounded text-[9px] font-mono font-extrabold uppercase border ${
                    ...
                    selectedBrandName === 'Apple' ? 'text-slate-900 bg-slate-500/10 border-slate-500/20' :
                ...
                <h2 className="text-3xl font-black font-display tracking-tight text-slate-900 uppercase mt-1">
              ...
              <div className="grid grid-cols-2 gap-3 bg-white/80 p-4 rounded-xl border border-slate-200">
                ...
                <div className="flex flex-col gap-3.5 max-h-[340px] overflow-y-auto pr-1">
                  {activeBrandProducts.map((prod) => {
                    ...
                    return (
                      <div
                        key={prod.id}
                        className="p-3 bg-white/45 rounded-xl border border-slate-200 hover:border-slate-200 transition-colors flex items-center justify-between gap-4"
                      >
                        <div className="flex items-center gap-3">
                          <img
                            src={prod.image}
                          ...
                          <div className="text-left w-36 sm:w-48 lg:w-40 xl:w-44">
                            <h4 className="text-xs font-bold font-display text-slate-900 truncate hover:text-[#06B6D4] leading-snug">
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className={`p-1.5 rounded-lg border cursor-pointer hover:bg-slate-50 transition-all ${
                              isFav ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-white/50 border-slate-200 text-slate-500 hover:text-slate-700'
                            }`}
                          >
                          ...
                          <button
                            ...
                            className="p-1.5 bg-white/50 hover:bg-slate-50 cursor-pointer text-slate-500 hover:text-[#06B6D4] rounded-lg border border-slate-200 transition-all"
                          >
                            <Eye size={12} />
                          </button>
                          ...
                          <button
                            ...
                            className="p-1.5 bg-blue-600 hover:bg-blue-500 text-white hover:text-white rounded-lg border border-blue-650 transition-all cursor-pointer scale-100 active:scale-95"
                          >
                            <ShoppingBag size={12} />
                          </button>
                        </div>
      ...
      <section className="bg-slate-50 border border-slate-200 p-8 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left mt-8">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-cyan-600/10 text-cyan-400 border border-cyan-500/20 rounded-xl shrink-0 hidden sm:block">
            <Award size={22} />
          </div>
          <div>
            <h4 className="text-sm font-bold font-display uppercase tracking-wider text-slate-900">Genuine Certification Tags</h4>
            <p className="text-xs text-slate-500 mt-1">Every partner product comes backed by official brand seals and direct-to-customer factory warranties.</p>
          </div>
        </div>

        <button
          onClick={() => navigateTo('shop')}
          className="px-5 py-3 bg-blue-600 hover:bg-blue-500 text-white hover:text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all flex items-center gap-1.5 cursor-pointer font-display font-black scale-100 hover:scale-103 duration-200"
        >
