...
export default function MyAccount() {
  ...
  if (!userProfile) {
    return (
      <div className="pt-[150px] pb-24 text-center max-w-sm mx-auto flex flex-col items-center gap-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-white font-display uppercase tracking-widest mb-2">Unauthenticated Terminal</h2>
        ...
        <button
          ...
          className="w-full py-3 bg-blue-600 text-slate-950 font-bold uppercase text-xs rounded-xl hover:text-white"
        >
  ...
  return (
    <div id="account-view-container" className="pt-[110px] pb-24 max-w-7xl mx-auto px-4 md:px-8 w-full flex flex-col md:flex-row gap-10">
      
      {/* LEFT ACCREDIT CONTROL COLUMN */}
      <aside className="w-full md:w-64 shrink-0 flex flex-col gap-4">
        
        {/* User Mini card layout */}
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl text-left flex items-center gap-4 relative">
          ...
          <div className="max-w-[140px] truncate">
            <h3 className="font-display font-extrabold text-sm text-white truncate">{profileName}</h3>
            <span className="text-[9px] font-mono font-bold text-emerald-400 uppercase tracking-widest">LOYAL MEMBER</span>
          </div>
        </div>

        {/* Navigation list */}
        <nav className="bg-slate-900 border border-slate-800 rounded-2xl p-3 flex flex-col gap-1.5 text-left">
            ...
            return (
              <button
                ...
                className={`w-full px-4 py-2.5 rounded-xl text-xs font-semibold uppercase tracking-wider flex items-center justify-between cursor-pointer transition-colors ${
                  isSel ? 'bg-blue-600 text-slate-950 font-black' : 'text-slate-400 hover:text-white hover:bg-slate-850'
                }`}
              >
          ...
          <button
            ...
            className="w-full px-4 py-2.5 rounded-xl text-xs font-semibold uppercase tracking-wider text-red-400 hover:text-white hover:bg-slate-800/40 flex items-center gap-3 cursor-pointer text-left"
          >
            <LogOut size={16} />
      ...
      <main className="flex-1 bg-slate-900/40 border border-slate-805 p-6 md:p-8 rounded-3xl min-h-[500px]">
        
        {/* ================= 1. DASHBOARD OVERVIEW ================= */}
        {activeMenu === 'dashboard' && (
          <div className="flex flex-col gap-8 text-left">
            <div className="border-b border-slate-855 pb-4">
              <h2 className="text-xl md:text-2xl font-bold font-display text-white uppercase tracking-wider">Welcome back, {profileName}!</h2>
              <p className="text-xs text-slate-400 mt-1">Ecosystem dashboard overview coordinates. Registered: <strong className="text-white font-mono">{profileEmail}</strong></p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              ...
              {/* Total Orders */}
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex flex-col gap-2 relative overflow-hidden">
                ...
                <span className="text-[11px] text-slate-400 font-sans mt-1">Sum: <strong className="text-white font-mono">${totalSpend.toFixed(2)}</strong> total</span>
              </div>

              {/* Wishlist Items */}
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col gap-2 relative overflow-hidden">
                ...
                <span className="text-[11px] text-slate-400 font-sans mt-1">Pinned drops waiting</span>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col gap-2 relative overflow-hidden">
                ...
                <span className="text-[11px] text-slate-400 font-sans mt-0.5">Approx. <strong className="text-white font-sans">${(userProfile.rewardPoints / 10).toFixed(2)}</strong> savings</span>
              </div>
            </div>

            {/* QUICK HISTOGRAM BRIEF */}
            <div className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl flex flex-col gap-4">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 font-display">Active operational notice</h4>
              <p className="text-xs text-slate-400 leading-relaxed">
                Your complimentary <strong className="text-white font-semibold">2-Year Mechanical Warranty</strong> is activated on checkouts. To claim support or report hardware faults, coordinate with staff coordinates inside our Contact HQ panel.
              </p>
            </div>
          </div>
        )}
        ...
        {activeMenu === 'orders' && (
          <div className="flex flex-col gap-6 text-left">
            <h2 className="text-xl font-bold font-display text-white uppercase tracking-wider border-b border-slate-800 pb-3">Purchase Order Archives</h2>
            ...
            {orders.length === 0 ? (
              ...
              <div className="overflow-x-auto border border-slate-800 rounded-xl bg-slate-950/30">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-900 text-slate-400 border-b border-slate-805 uppercase tracking-wider font-mono">
                  ...
                  <tbody className="divide-y divide-slate-855 text-slate-300">
                    {orders.map((ord) => (
                      <tr key={ord.id} className="hover:bg-slate-905/40">
                        ...
                        <td className="p-3.5 text-right font-bold font-mono text-slate-100">${ord.total.toFixed(2)}</td>
        ...
        {activeMenu === 'wishlist' && (
          <div className="flex flex-col gap-6 text-left">
            <h2 className="text-xl font-bold font-display text-white uppercase tracking-wider border-b border-slate-800 pb-3">My Saved Wishlist</h2>
            ...
            {wishlistProducts.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {wishlistProducts.map((p) => (
                  <div key={p.id} className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col justify-between group h-64">
                    <div className="bg-slate-950 p-3 rounded-lg flex items-center justify-center h-28 relative">
                      <img src={p.image} alt={p.name} className="max-h-full object-contain" referrerPolicy="no-referrer" />
                      <button
                        onClick={() => toggleWishlist(p.id)}
                        className="absolute top-2 right-2 p-1 bg-slate-900 hover:bg-slate-800 rounded-full text-slate-400 hover:text-red-400"
                        title="Remove"
                      >
                        <Trash2 size={12} />
                      </button>
                    ...
                    <div className="mt-2 text-left flex flex-col gap-1">
                      <h4 className="font-semibold text-xs text-white max-w-[150px] truncate">{p.name}</h4>
                      <p className="text-xs font-bold text-blue-400">${p.price}</p>
                      
                      <button
                        onClick={() => addToCart(p, 1)}
                        className="w-full mt-2 py-1.5 bg-blue-600 hover:bg-blue-500 text-slate-950 font-black text-[10px] uppercase tracking-wider rounded"
                      >
                        Quick Cart
                      </button>
                    </div>
        ...
        {activeMenu === 'addresses' && (
          <div className="flex flex-col gap-6 text-left">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h2 className="text-xl font-bold font-display text-white uppercase tracking-wider">Address Logs Registry</h2>
              <button
                onClick={() => setShowAddressForm(prev => !prev)}
                className="px-3 py-1.5 border border-slate-720 text-[10px] text-white font-bold rounded uppercase tracking-wider cursor-pointer"
              >
            ...
            {showAddressForm && (
              <form onSubmit={handleCreateAddress} className="p-4 border border-slate-750 bg-slate-950/40 rounded-xl flex gap-3">
                <input
                  type="text"
                  placeholder="Street Coordinate e.g. 520 Tech Ave Suite A"
                  value={addrFormLine}
                  onChange={(e) => setAddrFormLine(e.target.value)}
                  className="flex-1 bg-slate-900 border border-slate-700 text-xs text-white rounded-lg px-3 py-2"
                  required
                />
                <button
                  type="submit"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {addresses.map((addr) => (
                <div key={addr.id} className="p-5 bg-slate-900 border border-slate-800 rounded-2xl relative flex flex-col gap-3">
                  ...
                  <div className="text-xs leading-normal font-sans">
                    <p className="font-bold text-white text-sm mb-0.5">{addr.firstName} {addr.lastName}</p>
        ...
        {activeMenu === 'profile' && (
          <div className="flex flex-col gap-6 text-left max-w-xl">
            <h2 className="text-xl font-bold font-display text-white uppercase tracking-wider border-b border-slate-800 pb-3">Update Member Biography</h2>
            ...
            <form onSubmit={handleProfileSubmit} className="flex flex-col gap-4 text-xs">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="bg-slate-950 border border-slate-700 rounded-lg px-3.5 py-2.5 text-xs text-white"
                  required
                />
              </div>
              ...
              <div className="flex flex-col gap-1 mt-2">
                ...
                <input
                  ...
                  className="bg-slate-955 border border-slate-704 rounded-lg px-3.5 py-2.5 text-xs text-white"
                />
              </div>
